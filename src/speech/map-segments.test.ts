...
describe("mapWordsToSegments", () => {
  ...
  test("segments touch — no gaps between adjacent segments", () => {
    ...
    const first = atOrThrow(result, 0, "Expected first segment");
    const second = atOrThrow(result, 1, "Expected second segment");
    const third = atOrThrow(result, 2, "Expected third segment");

    // Adjacent segments should touch (no gap)
    expect(first.end).toBeCloseTo(second.start, 10);
    expect(second.end).toBeCloseTo(third.start, 10);
    // First starts at 0, last ends at audioDuration
    expect(first.start).toBe(0);
    expect(third.end).toBe(3.5);
  });
